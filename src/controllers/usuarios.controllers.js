...
const secret = "mysecret" // chave secreta
...
module.exports = {
  ...
  async create(req, res) {
    ...
    if (!user) {
      ...
      return res.status(303).json({ status: 303 })
    }
  },
  ...
  async login(req, res) {
    ...
    Usuario.findOne({ email_usuario: email, tipo_usuario: 1 }, (err, user) => {
      if (err) {
        console.log(err)
        res.status(200).json({ erro: "Erro no servidor. Por favor, tente novamente..." })
      } else if (!user) {
        res.status(200).json({ status: 2, error: 'E-mail não foi encontrado no banco de dados!' })
      } else {
        user.isCorrectPassword(senha, async function (err, same) {
          if (err) {
            res.status(200).json({ status: 2, error: "Erro no servidor. Por favor, tente novamente!" })
          } else if (!same) {
